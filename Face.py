...
class Face:
    ...
    def __init__(self, i, i2 = 0, image = [[[]]], x = -1, y = 0, w = 0, h = 0, name = ""):
        now = time.time()
        self.ID = i
        if (x == -1):
            ...
            cv2.imwrite("Data/OriginalFaces/" + name+ "_" + str(self.ID) + "_" + str(i2) +  ".jpg", self.orig)
            print "Crop/Scaled one face in:", time.time() - now
            self.grayFace = iManip.grayFace(self.orig)          #The grayscale face
            cv2.imwrite("Data/GrayFaces/" + name+ "_" + str(self.ID) + "_" + str(i2) + ".jpg", self.grayFace)
